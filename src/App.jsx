...
import { TEAM_SIZE, DEFAULT_ATK, DEFAULT_DEF, W, H } from './game/constants.js';
import {
  Volume2, VolumeX, RotateCcw, HelpCircle,
  Trophy, Skull, X, Shield, Swords, Eye, EyeOff
} from 'lucide-react';
      ...
      <div className="relative w-full max-w-[480px] h-full max-h-[920px] flex flex-col justify-between items-center shadow-2xl bg-[#080d14] border-x border-[#172331]">
        ...
        <main className="relative flex-1 w-full min-h-0 flex items-center justify-center p-2 overflow-hidden">
          ...
          >
            ...
            {/* ── Optional Floating Boss HP Bar (Monster Strike style) ── */}
            {boss && (
              <div className="absolute top-3 inset-x-3 pointer-events-none z-20 flex flex-col items-center">
                {showBossBar ? (
                  ...
                  <button
                    onClick={() => setShowBossBar(true)}
                    title="Show Boss HP Bar"
                    className="pointer-events-auto bg-[#0c131db0] backdrop-blur-md border border-slate-700/60 hover:border-[#ff2a55]/60 text-slate-300 hover:text-white px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold flex items-center gap-1 shadow-lg active:scale-95 transition-all"
                  >
                    <Eye size={12} className="text-[#ff2a55]" />
                    <span>BOSS HP</span>
                  </button>
                )}
              </div>
