...
| `422 Unprocessable Entity` | A structurally valid battle result fails deterministic replay validation. |
...
| `SettleBattle` | `ErrInvalidInput` | `500` | A trusted battle service constructs the settlement after replay validation. |
| `TransferUnit` | `ErrInvalidInput` | `500` | This is an internal operation, not a directly decoded HTTP request. |
...
player lock. Errors: `403 asset_not_owned`, `404 unit_not_found`,
`409 unit_unavailable`, and `500 internal_error` if the store still reports an
...
```json
{
  "battle_seed": "server-generated-seed",
  "dungeon": {
    "id": "uuid",
    "name": "First Dungeon",
    "enemy_config": []
...
```json
{
  "battle_seed": "server-issued-seed",
  "unit_snapshot": [],
  "action_log": [],
  "claimed_outcome": "won"
}
...
Handler validation checks presence, supported action types, collection limits,
and payload size. A battle service then replays deterministic combat. The
client does not submit trusted `player_id`, `money_award`, `treasure_drops`, or
final database updates. The validator constructs `domain.BattleResult` from
the authenticated player, verified replay, and server dungeon configuration.

On successful replay and settlement: `204 No Content`.

On replay mismatch, call `ResetPlayerStatus` and return:

```text
422 battle_result_mismatch
```

If resetting the status fails, log both errors and return `500 internal_error`;
do not report a successful reset.

Other errors: `404 dungeon_not_found`, `409 player_not_in_combat`, and
`409 battle_state_changed`.
...
`409 already_equipped`, and `409 player_busy` if the service prohibits loadout
